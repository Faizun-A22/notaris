...
export const DocumentDetailPage = () => {
  ...
  const getPTChecklistFallback = () => {
    return [
      ...
      { id: 13, name: 'FOTOKOPI BUKTI SETOR MODAL (BUKU TABUNGAN, REKENING KORAN, BUKTI TRANSFER KE REKENING AN. PERSERO)', desc: 'Bukti penyetoran modal ke rekening koran atas nama PT', status: 'Belum Diunggah' },
    ...
    const updatedChecklist = checklist.map((item) =>
      item.id === itemId 
        ? { 
            ...item, 
            status: newStatus,
            fileName: fileData ? fileData.name : (newStatus === 'Belum Diunggah' ? null : item.fileName),
            fileUrl: fileData ? fileData.url : (newStatus === 'Belum Diunggah' ? null : item.fileUrl)
          } 
        : item
    );
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant p-card-padding shadow-[0px_4px_20px_rgba(0,0,0,0.05)]">
            ...
            <div className="flex flex-col gap-base">
              {checklist.map((item, idx) => {
                ...
                const isPending = item.status === 'Belum Diunggah';
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3 min-w-0">
                      {isReceived ? (
                        <div
                          onClick={() => handleUpdateChecklistStatus(item.id, 'Belum Diunggah')}
          ...
          <div className="fixed inset-0 bg-inverse-surface/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white border border-outline-variant rounded-xl w-full max-w-lg p-6 relative shadow-xl text-left animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="mt-6 flex gap-3">
                <button
                  onClick={() => {
                    handleUpdateChecklistStatus(selectedDocForReview.id, 'Belum Diunggah');
